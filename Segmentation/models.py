...
def unet2D(pretrained_weights = None,input_size = (256,256,1),lr=1e-4):
    ...
    up6 = Conv2D(256, 2, padding = 'same')(padd)
...
# # Construct 3D Unet Model 
# 
# Usage:
# 
#     model = unet3D(input_size = (80, 120,120,1), lr=1e-4)
...
def unet3D(pretrained_weights = None,input_size = (80,120,120,1), lr=1e-4):
    input_size = input_size + (1,)
    inputs = Input(input_size)
...
#    padd = ZeroPadding3D(((0,0),(0,1),(0,1)))(UpSampling3D(size = (2,2,2))(drop4))
    padd = UpSampling3D(size = (2,2,2))(drop4)
    # --------------------------------------------------------------------------------------------------------
...
def train_model(model,training_generator,testing_generator, name="model", epochs=1):
    filepath = name + "_{epoch:03d}-{loss:.4f}.h5"

    checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1, save_best_only=False, mode='min')
    csv_logger = CSVLogger(name + "_history_log.csv", append=True)
